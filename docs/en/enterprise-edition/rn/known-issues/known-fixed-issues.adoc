...
|You may notice a few alerts for *Attack Path* policies are in open state when they should be marked as resolved. This was due to an issue that is now fixed, however, we are still working on bringing the impacted alerts to the right state. 
